...
2. **Create a tunnel** з окремою назвою (`rich-studio`), скопіюйте token.
   ...
   конектор мертвий.
3. У старому тунелі **видаліть public hostname** цього домену. Поки він там,
   Cloudflare не дасть створити такий самий маршрут у новому тунелі.
4. Зупиніть старий конектор на його хості.
5. У новому тунелі → **Public hostname**: hostname порожній (це apex-домен,
   підпапка не потрібна), Service **HTTP**, URL `web:80`.
   Cloudflare сам створить проксійований CNAME на apex.
6. Далі — кроки 3-6 вище (token у `.env`, профіль `tunnel`, OAuth, media).

DNS-запис apex від старого проєкту (A або CNAME) видаляти вручну НЕ треба:
майстер public hostname перезапише його сам. Якщо він поскаржиться на
конфлікт — значить крок 3 пропущено.
