...
### Домен уже зайнятий іншим проєктом

Найнебезпечніша пастка — **не** DNS, а реплікація конектора. Токен тунелю
прив'язує до нього КОЖЕН запущений `cloudflared`. Якщо конектор старого
проєкту живий десь іще і ви піднімете другий з тим самим токеном, Cloudflare
почне розкидати запити між обома: половина відвідувачів побачить стару
систему, і жоден лог на TrueNAS цього не покаже.

Тому надійний порядок — окремий тунель під студію, а старий гаситься цілком:

1. **Zero Trust → Networks → Tunnels**. Подивіться на старий тунель: у нього
   видно активні конектори і хости, на яких вони крутяться. Запам'ятайте, де
   саме працює старий `cloudflared` — його треба буде зупинити руками.
2. **Create a tunnel** з окремою назвою (`rich-studio`), скопіюйте token.
   Не перевикористовуйте старий token, навіть якщо здається, що старий
   конектор мертвий.
3. **Зніміть конфлікт за адресою.** Майстер public hostname НЕ перезаписує
   чужий запис - він відмовляє помилкою «A DNS record with this name already
   exists». Спосіб залежить від того, чим віддається старий проєкт:
   - старий тунель → Zero Trust → Networks → Tunnels → старий тунель →
     **Public Hostname** → видалити запис домену. Так зникне і DNS-запис;
   - звичайний запис (A/CNAME на інший сервер, Pages, хостинг) → головна
     панель Cloudflare → домен → **DNS → Records** → запис з іменем домену
     (може показуватись як `@`) → **Delete**.

   Дивіться ОБИДВА місця: якщо запис створив тунель, видалення лише з DNS
   лишає маршрут, і майстер упреться знову.
4. Зупиніть старий конектор на його хості.
5. У новому тунелі → **Public hostname**: Subdomain порожній (це apex-домен),
   Path порожній, Service **HTTP**, URL рівно `web:80`.
   Cloudflare сам створить проксійований CNAME на apex.
6. Далі — кроки 3-6 вище (token у `.env`, профіль `tunnel`, OAuth, media).

**URL сервісу - це `web:80`, а не LAN-адреса хоста.** `192.168.x.x:3000`
теж «якось працює»: запит виходить з контейнера на опублікований порт хоста і
повертається всередину. Але це зайвий гак через мережу хоста, який ламається
від будь-якої зміни LAN-адреси чи проброшеного порту. `cloudflared` живе в тій
самій compose-мережі, що й `web`, і ходить до nginx прямо за іменем сервісу.

### Що змінює публічний домен у самому застосунку

Це вже зроблено в конфігах, але знати варто — при наступній правці nginx легко
зламати обидва пункти назад.

- **Справжній IP відвідувача.** За тунелем `$remote_addr` — це адреса
  контейнера `cloudflared`, одна на всіх. Ліміт входу «10 спроб на e-mail і 30
  на IP за 5 хвилин» рахував би відвідувачів разом: один перебірник вибиває
  решту, а сам лишається невидимим. `real_ip_header CF-Connecting-IP` з довірою
  лише до приватних мереж повертає лічильнику справжню адресу.
- **Схема оригінального запиту.** TLS терминується в Cloudflare, до nginx усе
  приходить по http. Без `X-Forwarded-Proto` і `--proxy-headers` у uvicorn
  `request.base_url` збирав би `http://` — і OAuth-провайдер відбивав би
  `redirect_uri` як невалідний. Явні `GITHUB_CALLBACK_URL`/`GOOGLE_CALLBACK_URL`
  лишаються головним механізмом, це — страхувальна сітка.

### Стеля Cloudflare у 100 секунд

Cloudflare рве запит на 100 секундах (помилка **524**) — це не налаштовується
на безкоштовному тарифі. Генерація річів це не зачіпає: вона йде в чергу Celery,
а сторінка опитує статус короткими запитами.

Під стелю можуть підійти синхронні виклики моделі: «Критик», «Виправити тексти»,
генерація стилю та «Покращити стиль». Одне звернення обмежене
`REQUEST_TIMEOUT_SECONDS=75`, тож запас невеликий. Найризикованіший — **«Блоки
в PNG»** для сторінки з великою кількістю блоків: Chromium знімає кожен окремо
(профіль `shots`, типово вимкнений). Якщо він знадобиться регулярно — робіть це
через LAN-адресу `http://IP:3000`, повз тунель.
...
## Гейт верстки (та сама служба `shots`)

Той самий Chromium робить другу, важливішу роботу. Наприкінці кожного прогону
воркер відкриває готові сторінки - десктоп у 1240px, мобільну в 390px - і **міряє
укладену верстку**: чи не обрізаний текст, чи не перекритий він сусіднім блоком,
який контраст виходить проти НАМАЛЬОВАНОГО під ним тла, чи не зʼявився
горизонтальний скрол, чи всі зображення завантажились, чи не поїхали ресурси
повз студію. Результат лягає окремою карткою «Верстка у браузері» у вкладці
«Якість», з фрагментом тексту біля кожної знахідки.

Навіщо окремо від решти рецензентів: ті читають розмітку й текст. Обрізаний
заголовок, цифра, що злилася з плиткою, сторінка, яка їде вбік на телефоні -
усе це існує лише після укладання, і в розмітці його не видно в принципі.

Без запущеного `shots` прогін іде як раніше, але **перевірки верстки не буде**:
воркер пише про це в журнал проєкту, а у вкладці «Якість» зʼявляється червона
примітка. Тому на робочому сервері службу варто тримати піднятою:

```sh
docker compose --profile shots up -d --build shots
docker compose ps shots
```

Аудит нічого не коштує в токенах (свій браузер), додає до прогону ~5-10 секунд
на сторінку і ніколи не валить проєкт: недоступна служба - це попередження, а
не помилка.
