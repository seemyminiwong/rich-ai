...
checks.update({
    ...
    'image link swap: one place to repoint every frame at the shop server': 'def image_urls_in_html' in pipeline and 'def replace_image_urls' in pipeline and 'def is_publishable_image_url' in pipeline and "/api/artifacts/{artifact_id}/images" in main and 'image_map_json' in main and 'image_map_json' in models and (root / 'apps/api/alembic/versions/0019_project_image_map.py').exists() and 'function imageLinksPanel' in web and 'applyImageLinks' in web and "t.push('links')" in web,
    ...
    'mobile hero shows the whole product without dead space': 'def _fit_mobile_hero' in pipeline and '_MOBILE_HERO_TOP = 420' in pipeline and 'mobile 420px 18px 26px' in prompts and "variant == 'mobile'" in pipeline and 'background-size:100% auto' in pipeline and "';aspect-ratio:2/3'" not in pipeline and '_fit_mobile_hero(relaid' in tasks,
