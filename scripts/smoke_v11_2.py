...
checks.update({
    'video block: youtube only, server-injected, switchable by link': 'def youtube_video_id' in pipeline and 'def inject_video_block' in pipeline and "youtube-nocookie.com/embed/" in pipeline and 'video_url' in models and (root / 'apps/api/alembic/versions/0018_project_video.py').exists() and 'video=getattr(project' in tasks and 'inject_video_block(' in tasks and "name=\"video_url\"" in web and 'video_url' in main,
