...
checks.update({
    'video block: youtube only, server-injected, switchable by link': 'def youtube_video_id' in pipeline and 'def inject_video_block' in pipeline and "youtube-nocookie.com/embed/" in pipeline and "srcdoc'] =" not in pipeline and "'srcdoc'" not in pipeline.split('_ALLOWED_ATTRS')[1][:200] and 'videoPlaceholderForPreview' in web and 'i.ytimg.com/vi/' in pipeline and 'padding-top:56.25%' in pipeline and '_VIDEO_WATCH' in pipeline and 'video_url' in models and (root / 'apps/api/alembic/versions/0018_project_video.py').exists() and 'video=getattr(project' in tasks and 'inject_video_block(' in tasks and "name=\"video_url\"" in web and 'video_url' in main,
    'paid feature is always mounted, env frames cover, faq starts closed': 'def ensure_feature_mounted' in pipeline and 'ensure_feature_mounted(relaid, feature' in tasks and '_is_environment_photo' in pipeline and '_probe_media_surface' in pipeline and "item.attrs.pop('open', None)" in pipeline and 'never write the open attribute' in prompts,
    ...
    'critic keeps up with server animations and multi-page brand counting': 'risky_css' in pipeline and '12 * max(1, len(artifacts))' in pipeline and "Style tag detected; inline CSS is required" not in pipeline,
